...
\hint{This chapter existed in the version 1 original .tex files but was not publised in all languages. It needs to be reviewed and completed at least for Linux users.}
...
Next, let's unzip the wx sources into \file{C:\osp Projects} so we will end up with a wx root directory like this: \file{C:\osp Projects\osp wxWidgets-3.1.3}. Next unzip the patch into the same directory letting it overwrite files. Note that we are going to refer to the wx root directory from now on as \var{wxDir}
